...
If input is the narrowPeak file from Genrich, the output has header:
...
parser.add_argument('--narrowPeak', '-n',
                    ...
                    metavar= 'FILE',
                    help='''\
Input peak file in bed format. Typically this is the narrowPeak file from Genrich''')
...
if __name__ == "__main__":
    ...
    #   chr     start   end     experimental    control -log(p)
    #
    np = open(args.narrowPeak).readlines()
        ...
        for line in bdg:
            ...
            if line.startswith('#'):
                if first:
                    ...
                    peak_logfc[bdg.experiment] = bdg.logfc(args.narrowPeak)
                    bdg = Bdgfile(line)
                continue
    ...
    extend_narrowPeak(args.narrowPeak, peak_logfc)
