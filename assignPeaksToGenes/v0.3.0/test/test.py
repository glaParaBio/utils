...
class Test(unittest.TestCase):
    ...
    def testColumnNames(self):
        ...
        self.assertEqual(0, p.returncode)
        bed = pandas.read_csv('test_out/out.bed', sep= '\t')
        header = '#foo_chrom foo_start foo_end foo_id foo_distance foo_strand foo_n_intra_tss foo_best_picks start end peak_id'.split(' ')
    ...
    def testBestPicks(self):
        p = sp.Popen("./assignPeaksToGenes.R -n 3 -s '' -gff test/data/annotation.bed -p test/data/peaks2.bed > test_out/out.bed", 
                shell=True, stdout= sp.PIPE, stderr= sp.PIPE)
        stdout, stderr = p.communicate()
        self.assertEqual(0, p.returncode)
        bed = pandas.read_csv('test_out/out.bed', sep= '\t')
        self.assertEqual(len(bed[bed.feature_distance.isna()]), len(bed[(bed.feature_distance.isna()) & (bed.feature_best_picks.isna())]))
        
        self.assertEqual('peak_3', bed[(bed.feature_id == 'PBANKA_0941500.1') & (bed.feature_best_picks == True)].peak_id.iloc[0]) 
        self.assertEqual('peak_2', bed[(bed.feature_id == 'PBANKA_0938200.1') & (bed.feature_best_picks == True)].peak_id.iloc[0]) 

        bed = bed[(bed.feature_distance.isna() == False)]
        self.assertEqual(sorted(list(bed[(bed.feature_best_picks == True)].feature_id)), sorted(list(set(bed.feature_id))))

    def testBestPicksWithDuplicates(self):
        p = sp.Popen("./assignPeaksToGenes.R -n 3 -s '' -gff test/data/annotation.bed -p test/data/duplicates.bed > test_out/out.bed", 
                shell=True, stdout= sp.PIPE, stderr= sp.PIPE)
        stdout, stderr = p.communicate()
        self.assertEqual(0, p.returncode)
        bed = pandas.read_csv('test_out/out.bed', sep= '\t')
        # best_picks is NA if feature_distance is NA
        self.assertEqual(len(bed[bed.feature_distance.isna()]), len(bed[(bed.feature_distance.isna()) & (bed.feature_best_picks.isna())]))
        
        self.assertEqual(2, len(bed[(bed.feature_id == 'RNA1.1') & (bed.feature_best_picks == True)]))
        self.assertTrue(len(bed[(bed.feature_id == 'RNA1.1') & (bed.feature_best_picks == False)]) > 2)
