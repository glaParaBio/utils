...
peak_rank <- function(distance) {
    if(length(distance) == 1 & is.na(distance[1])) {
        return(as.integer(NA))
    }
    stopifnot(!any(is.na(distance)))
    names(distance) <- 1:length(distance)
    xrank <- c(-rank(abs(distance[distance <= 0]), ties.method='first'),
               rank(distance[distance > 0], ties.method='first')
               )
    xrank <- xrank[order(names(xrank))]
    return(as.vector(xrank))
}

pick_best <- function(distance){
    if(length(distance) == 1 & is.na(distance)[1]) {
    ...
    drank <- rank(abs(distance), ties.method='first') * sign(distance)
    best_neg <- ifelse(all(distance > 0), NA, which.min(1/drank))
    best_pos <- ifelse(all(distance < 0), NA, which.max(1/drank))
    choice <- rep(FALSE, length(distance))
    if(is.na(best_neg)) {
        choice[best_pos] <- TRUE
    } else if(is.na(best_pos)) {
        choice[best_neg] <- TRUE
    } else if(abs(distance[best_neg]) <= abs(distance[best_pos])){
        choice[best_neg] <- TRUE
    } else {
        choice[best_pos] <- TRUE
    }
    stopifnot(sum(choice) == 1)
    stopifnot(length(choice) == length(distance))
    return(choice)
}
...
xclosest[, best_picks := pick_best(distance), by= id]

name_order <- c('#chrom', 'start', 'end', 'id', 'distance', 'strand', 'n_intra_tss', 'best_picks')
