...
parser <- ArgumentParser(description= 'Assign to each feature in GFF the nearest peak')
parser$add_argument('--peaks', '-p', help= 'Peak file in bed-compatible format [%(default)s]', default= "-", metavar= 'FILE')
parser$add_argument('--gff', '-gff', help= 'Annotation file in GFF format [%(default)s]', default= '-', metavar= 'FILE')

def <- 'mRNA'
parser$add_argument('--feature-type', '-f', help= 'Feature type from 3rd column in GFF identifying features to be annotated. Typically use "gene" or "mRNA"', default= def, metavar= sprintf('[%s]', def))

def <- 'ID'
parser$add_argument('--gene-key', '-k', help= 'Attribute key in the gff file of the feature to use for annotation. Typically "ID"', default= def, metavar= sprintf('[%s]', def))

def <- c('description')
parser$add_argument('--extra', '-x', help= sprintf('Extra GFF attributes to output [%s]', paste(def, collapse= ' ')), default= def, metavar= 'NAME', nargs= '*')
...
closest <- function(peaks, tss, verbose= FALSE, tmpdir) {
    ...
    cmd <- sprintf('set -e \
    closestBed -a %s -b %s -D a > %s', tss_file, summit_file, out_intx)
